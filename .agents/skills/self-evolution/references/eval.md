...
- **免 key 可跑**：默认走 eval 子 agent（`.claude/agents/eval.md`，Codex 用 `.codex/agents/eval.toml`），免 API key；`make eval`（`scripts/run-eval.sh`）是可选 CI/headless 路径。两条路写同样的 `task-reviews/` 产出，Stop hook 只认产出在不在。
...
- **指针凭记忆迁移、悬空**（`lessons.md` 2026-06-26）：规则分布化时给 rule-0005/0006/0008 编了不存在的 eval 指针（005/006/008）、rule-0007 severity 私改，eval 子 agent 逐条 `git show HEAD` 对比判 yellow。**修复固化：把"eval 标记必须指向存在考题"加进 `rules-index.sh --check` 并变异自证。**
...
- **加/改考题或补规则守护** → `add-rule` skill（定范围→写下来+登记→挂执行），新增考题后跑 `verify-eval-materials.sh` 确认双向登记。
- **重生成规则索引 / 校验 eval 指针** → `bash scripts/rules-index.sh`（重生成）/ `--check`（只校验，进 `make verify`）。
- **补评 / 重评** → eval 子 agent（`.claude/agents/eval.md`，免 key）；CI/headless 用 `make eval ARGS="..."`。产出落 `docs/eval/task-reviews/`。
- **结构自检** → `bash scripts/verify-eval-materials.sh`（`make verify-eval`）；整体控制面自检 `make verify`。
- **发现晋升归档** → `self-evolution` skill / `self-optimize` 子 agent（把捞到的缺口/漏洞从 log 晋升到规则/考题/lessons，不许烂在 `tasks/optimization-log.md`）。
