...
- **补/改规则索引**：改 `AGENTS.md` 的 `<!-- rule: -->` 标记后 `bash scripts/rules-index.sh` 重生成；走 `add-rule` skill 保证「写下来 + 登记 + 挂执行」三步齐。
...
- **统筹与登记缺口**：用 `self-evolution` skill 把发现晋升归档；机器查不了的判断写 `tasks/optimization-log.md`，捞到的须晋升、不许烂在 log 里（rule-0011）。
