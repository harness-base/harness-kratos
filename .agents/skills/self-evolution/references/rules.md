...
- **加/改/晋升一条规则** → `add-rule` skill（`.agents/skills/add-rule/SKILL.md`，version 2）：三步——①定范围（放最浅可覆盖 `AGENTS.md`）②写 bullet + 标记 + `bash scripts/rules-index.sh` 重生成 catalog ③挂执行（能机器判进 `hook-policy.sh`+`.test.sh`，要人判进 `docs/eval/prompts/`+`index.yaml`）。
