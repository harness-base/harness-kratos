...
- **反复出现的 lesson 必须晋升成规则**（走 `add-rule`），不许同型坑反复记不收口（rule-0007）。
- `tasks/optimization-log.md` 是**中转站**：兜底（Stop hook 机械触发）和 judgment（self-optimize 子 agent）捞到的候选落这里，**重要的必须晋升到家**——决策→ADR、踩坑→`lessons`、知识→就近 `AGENTS.md`/规则、偏好→memory（rule-0011）。log 里不许烂着不动。
...
- **反复 lesson → 晋升成规则**：用 `add-rule` skill（`.agents/skills/add-rule/SKILL.md`）——定范围→写进就近 `AGENTS.md`+登记→挂执行/考题。
- **处理 log 候选 / 主动深审**：用 `self-evolution` skill（`.agents/skills/self-evolution/SKILL.md`，规范检查层入口）；复杂时 spawn `self-optimize` 子 agent（`.claude/agents/self-optimize.md`）。把候选搬到 ADR/lessons/就近 AGENTS.md/规则/memory，不许烂在 log（rule-0011）。
