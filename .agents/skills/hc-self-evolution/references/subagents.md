...
- **职责与 skill 分清**：`skill = 流程`（主 agent 读了照着走），`subagent = 独立执行体`（被 spawn 去干一件隔离的事）。skill 与 subagent 是**入口 vs 引擎**关系，不是两份重复逻辑——如 `hc-self-evolution` skill（深审入口、列总览索引、晋升归档）调 `hc-self-optimize` 子 agent（多维判断引擎）。
