...
- 流程之间**接力关系明确**：上下游写在 description / 正文里（如 `hc-prd`【上游】→ `hc-dev`），不靠 agent 猜。
...
- **有流程吗**：该任务类型在 `.agents/skills/README.md` 有 skill，**或** AGENTS.md 有明文步骤 → 符合；两处都没有 → **缺口**（靠临场）。
  - 现状判例（核对过）：`产出需求`=hc-prd✓ / `实现需求`=hc-dev✓ / `产出用例`=hc-test✓ / `评审收尾`=rule-0005 + hc-eval 子 agent（成文，非 skill，可接受）/ `bugfix`=hc-dev✓（改 bug 子模式）/ `重构`=hc-dev✓（写代码统一入口）/ `迁移`=hc-dev✓（深度级迁移子模式：逐条核源 / 禁凭记忆）/ `loop-engineering`=**缺口**（仓内无 loop skill）。
- **门禁挡在临场之前吗**：高返工任务的流程是不是 STOP-before-do（先立项/先有证据再动手）？只有事后清单、没有事前闸 → **缺口**。
- **旧流程过时吗**：某流程的范式被更好的取代却没退场 → **漏洞**。已处理一例：`context-loading` 作为 skill 已降级为政策（ADR-0011——advisory 壳、不占触发/产物/闸，入口落 `AGENTS.md`）。仍开放的更大候选：L0-L6 通用档位 → 是否该演进成"按任务场景（bugfix/迁移/评审…）直接进对应 workflow-skill 并带好该场景的固定上下文"。判据：若 agent 经常"定了档却仍漏读场景专属规则"，说明通用档位粒度太粗。
...
- **某任务类型无流程 → 新建 skill**：用 `skill-creator`（写 `.agents/skills/<name>/SKILL.md`），按 self-evolution-plan「构建步骤」grounded 写，写完跑 `bash scripts/skills-index.sh`（重生成 `README.md`）→ `make verify` 收口（含 `skills-index --check`）。优先补：loop-engineering 流程（bugfix / 重构 / 迁移已由 `hc-dev` 填，ADR-0009）。
...
- **流程成文但不必成 skill（如收尾评审）→ 写进 AGENTS.md**：用 `hc-add-rule` 把步骤落成就近规则 + 登记 catalog，避免污染常驻技能列表。
