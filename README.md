...
- **技能集**：`dev`（写代码统一入口）、`test-case`（测试用例 + 覆盖硬闸）、`prd-elicitation`（编排式产需求）、`context-loading`、`doc-sync`、`self-evolution` 等——清单以 [`.agents/skills/README.md`](.agents/skills/README.md) 自动索引为准。
