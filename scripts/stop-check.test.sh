...
# 9) 当前节标题含 "reviewer"（review 子串）+ 无 Review 段 → 不拦（exit 0）。
#    钉死"review 须锚整词、不匹配 reviewer 子串"（杀 .*review 子串回归，lessons 2026-06-29）。task=foo2 无 eval。
printf '> 元：level: L3 ｜ task: foo2\n## 当前：搞 doc-sync reviewer\n- [ ] 实现中\n' > "$tmp/t9.md"
[ "$(run "$tmp/t9.md")" = "0" ] && ok || no "当前节标题含 reviewer 被误判收尾（review 不该匹配 reviewer 子串）"

# 10) "## 已闭" 归档块里有旧 ## Review → 不拦（exit 0）。钉死 已闭 是归档边界（杀边界漏认 已闭 回归）。task=foo2 无 eval。
printf '> 元：level: L3 ｜ task: foo2\n## 当前\n- [ ] 实现中\n\n## 已闭\n旧任务\n## Review\n- 旧小结\n' > "$tmp/t10.md"
