...
- 包含：
  ...
  - 入站 Kratos 中间件链：recovery / tracing(otel) / logging / metadata / metrics(prom) / ratelimit / validate。（熔断**不**在入站中间件，而是数据/客户端层 `sre.Breaker`，包住 PG/Redis/MQ 调用——见 ADR-0002 策略表与 `app/demo/internal/data`。）
...
- **AC6（网关链路）**：HTTP 经 gateway 到达 demo gRPC 服务并正确返回；入站中间件链（recovery/trace/log/metadata/metric/ratelimit/validate）生效。（熔断属数据/客户端层 `sre.Breaker`，不在此入站链，其行为由 AC3 验证。）
