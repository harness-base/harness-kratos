...
- 被管工程 `projects/kratos-base`：新增 `pkg/redisx`；demo 加 hits 接口（proto/biz/data/service）；conf/runtime/sandbox 扩展；readiness 加 redis。
- 受影响 skill（rule-0007）：feature-delivery / context-loading 无需更新。

## 测试设计

- 单测：redisx（Fingerprint 稳定/变更、不可达→error、Health=PING、addrs→模式选择）；hits biz/service（redis 不可用→结构化错误）。
- E2E（sandbox 加 redis）：`scen_redis_boot_down`(AC-R1)、`scen_redis_recover`(AC-R2)、`scen_redis_drop`(AC-R3)；回归跑 S0 `run_all.sh`(AC-R5)。
- 证据按 `workspace/verification.yaml` 路由记录。
