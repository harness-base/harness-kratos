...
| `.agents/skills/prd-elicitation/SKILL.md` | 编排「总谱」（worker 表 + 权重模型 + 时序 + 两 review 点 + 用户覆盖/留痕 + 门禁 + 演进） | 重写 |
| `.claude/agents/prd-reviewer.md` | PRD 审稿员（对抗挑刺，rubric=eval 013/rule-0010；轻审地基 / 重审下游） | 新建 |
| `.claude/agents/requirements-gatherer.md` | 需求采集员（多轮对话收需求） | 新建 |
| `.claude/agents/user-story-writer.md` | 用户故事+AC 员（写 user-stories.md） | 新建 |
| `.claude/agents/prd-writer.md` | PRD 本体员（写 prd.md） | 新建 |
| `.claude/agents/feature-point-writer.md` | 功能点清单员（FP + 映射） | 新建 |
| `.claude/agents/prototype-builder.md` | 原型员（可点原型） | 新建 |
...
| `.agents/skills/prd-elicitation/references/orchestration-workflow.js` | Workflow 编排模板（总监逻辑：时序/并行/确认门/review loop/条件触发可选 worker/每 worker 配 model+prompt） | 新建 |
...
- Rewrite: `.agents/skills/prd-elicitation/SKILL.md`
...
- [ ] **Step 3: 重写 `.agents/skills/prd-elicitation/SKILL.md` 成总谱**
...
```bash
git add docs/decisions/0010-prd-orchestration.md docs/decisions/index.yaml .agents/skills/prd-elicitation/ docs/superpowers/specs/
...
- Create: `.codex/agents/prd-reviewer.toml`
- Modify: `.codex/config.toml`

- [ ] **Step 1: 写 `.claude/agents/prd-reviewer.md`**
...
- [ ] **Step 2: 写 `.codex/agents/prd-reviewer.toml`**
...
config_file = "agents/prd-reviewer.toml"
```
...
```bash
git add .claude/agents/prd-reviewer.md .claude/agents/README.md .codex/agents/prd-reviewer.toml .codex/config.toml
...
**Files:**
- Create: `.agents/skills/prd-elicitation/references/orchestration-workflow.js`
...
Run: `node --check .agents/skills/prd-elicitation/references/orchestration-workflow.js 2>&1 || echo "需 node；或人工核 meta 字面量 + 无 TS 语法"`
Expected: 无语法错（meta 是纯字面量、无 `Date.now()`/`Math.random()`）。

- [ ] **Step 3: Commit** — `git add .agents/skills/prd-elicitation/references/ && git commit -m "prd 编排: Workflow 编排模板(总监逻辑)"`。
