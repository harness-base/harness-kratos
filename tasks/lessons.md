...
- Mistake：这轮往根 README 加 license / 参与贡献节，我**通读了整个 README** 却没发现 / 没提它一堆已过时（kratos 早挂了却仍写"以后挂进 `projects/`"、起步不全、context-loading"L0-L6"口径已漂、接入流程用户准备重构）。用户替我点出"这好像是文档漂移了"。我把视野缩在"加我这段"，没把"这文件整体还准不准"纳进来——尤其它**刚转公开、是门面**。
