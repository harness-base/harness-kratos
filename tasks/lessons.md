...
## 2026-06-26：macOS 没有 timeout/gtimeout，钩子里别用
...
## 2026-06-26：机械触发用"绝对阈值"会在大改动未提交时每轮误触发
...
## 2026-06-26：声称"无损迁移/全保留"却实际偷改，被独立 eval 抓出
...
## 2026-06-26：rule-0007 改了 skill 却没在 ADR 记录 = 判失败
...
## 2026-06-26：bash 用 TAB 作 IFS 分隔会折叠空字段
...
## 2026-06-24：多轮对抗评审（独立证伪 + 迭代到零）能抓出单轮漏掉的，包括修复自身引入的回归
...
## 2026-06-23：弹性 builder 忽略调用方 ctx，把"热更落地"的证据搞成脆弱的超时巧合
...
## 2026-06-23：共因污染断言——"坏 DSN + 停 pg → 503"无法区分热更落地与 pg 宕
...
## 2026-06-12：e2e 日志断言被"访问日志回显"骗过，掩盖了 100% 消息丢失
...
## 2026-06-12：池类依赖会掩盖"探活失败不标记重建"的缺口
...
## 2026-06-02：e2e 脚本隐含 CWD 假设，登记的验证命令实际是坏的
...
## 2026-06-02：Kratos config.Scan 走 json.Unmarshal，缺 json tag 的字段静默零值
...
## 2026-06-02：子代理长任务流式超时（idle timeout / 大依赖下载阻塞）
